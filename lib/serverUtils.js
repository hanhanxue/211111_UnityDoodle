...
export const getCustomMetadata = async (dir) => {
    ...
    const markdown = C_doodleDir.filter(el => el.match(/.mdx/g) !== null)[0]
    ...
    customMetadata.MDXSource = await getMDXSource(dir, 
        {defaultDimensions: {        
...
const getMDXSource = async (dir, scopedCustomData) => {
    ...
    const filepath = path.join(P_ABS_doodleDir, markdown)
